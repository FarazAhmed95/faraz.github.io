...
for f in features:
    ...
    if '<SimpleData name="Layer">GEUL</SimpleData>'.lower() in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FFFF0000' #blue
    if '<SimpleData name="Layer">TUINBORING</SimpleData>'.lower() in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FFff8e6c' #lightblue
    if '<SimpleData name="Layer">GESTUURDE BORING</SimpleData>' in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FF006400' #darkgreen
     
    if '<SimpleData name="Layer">Persing</SimpleData>' in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FF90ee90' #lightgreen
    if '<SimpleData name="Layer">Mentelbuis</SimpleData>' in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FF008000' #green
    if '<SimpleData name="Layer">Handhole</SimpleData>' in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FF00008b' #darkred
    if '<SimpleData name="Layer">DP</SimpleData>' in f_str :
        list((list(f.styles())[0]).styles())[0].color = 'FF0000ff' #red
    
   
#f = open("E:\\FIVERR\\jasper_1977\\testdwg_laptop\\out\\python_converted.kml", "w")
print('step3: setting colors')
...
print("***************")
print('Done: Output file' + kml_file)
